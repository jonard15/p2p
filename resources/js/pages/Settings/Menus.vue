                  ...
                  <!-- <draggable v-model="menus" @change="updateSorting">
                    <transition-group>
                      ...
                      >
                        ...
                        :key="child.menu_id"
                        v-if="child.parent_id === item.parent_id"
                            class="mx-auto mt-2"
                            max-width="500"
                            color="#1F7087"
                            dark
                          >
                            <v-card-actions>
                              <v-card-text class="font-weight-light">
                                ...
                                {{child.title}}</v-card-text
                              >
                              <v-spacer></v-spacer>
                              <v-btn icon @click="editItem(child)"
                                ><v-icon>mdi-pencil</v-icon></v-btn
                              >
                            </v-card-actions>
                          </v-card>
...
export default {
    ...
    computed : {
      ...
      filteredMenu(){
          return this.menus.filter(menu => menu.parent_id === 0 && menu.isActive === 1)
      },
      filteredChildMenu(){
          return this.menus.filter(menu => menu.parent_id != 0 && menu.isActive === 1)
      },
